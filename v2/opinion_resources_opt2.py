...
def get_structural_mask_for_gamma(gamma):
    P,M,K = gamma.shape
    # which entries are active for structural regularization
    mask = np.zeros(shape=gamma.shape)
    for i in range(K//2):
        mask[0, 2*i, list(range(K//2,K))] = 1
        mask[0, 2*i+1, list(range(0,K//2))] = 1
    for i in range(K):
        mask[1, i, [j for j in range(M) if j not in [i//2, (i//2) + (K//2)]]] = 1
    return mask

def get_structural_cost_for_gamma(gamma, mask):
    return np.sum(np.abs(gamma * mask))

def get_structural_gradient_for_gamma(gamma, mask):
    return np.sign(gamma * mask).reshape(-1)
...
def unflatten_parameters(plist, P, M, K):
    ...
    beta = plist[P*M+P*M*K: P*M+P*M*K+P*P*M*M].reshape(P,P,M,M)
...
def return_gradient_of_T_p_i_wrt_mu_hat(i, p, mu_hat, beta, lambda_indpt, mu, ext, N_mean, N_scale):
    ...
    for q in range(P):
        for j in range(M): 
            collector[q, j] = (beta[p,q,i,j]/N_scale[q,j]) * (1 / (lambda_indpt[q,j]+1)) * mu[q] * ext
...
def return_gradient_of_T_p_wrt_beta(i, p, t, mu_hat, beta, lambda_indpt, N_mean, N_scale):
    ...
    for r in range(P): 
            for k in range(M):
                collector[p,r,i,k] = (np.log(lambda_indpt[r,k]+1) - N_mean[r,k]) / N_scale[r,k]
    return collector.reshape(-1)

def return_gradient_of_T_p_i(i, p, t, mu_hat, beta, gamma, xf, lambda_indpt, mu, ext, N_mean, N_scale, X_mean, X_scale):
    wrt_mu_hat = return_gradient_of_T_p_i_wrt_mu_hat(i, p, mu_hat, beta, lambda_indpt, mu, ext, N_mean, N_scale)
    wrt_gamma = return_gradient_of_T_p_i_wrt_gamma(i, p, t, gamma, xf, X_mean, X_scale)
    wrt_beta = return_gradient_of_T_p_wrt_beta(i, p, t, mu_hat, beta, lambda_indpt, N_mean, N_scale)
...
def return_likelihood_and_gradient(    
    ...
    log_likelihood = 0
    gradient = np.zeros(shape = len(mu_hat.reshape(-1)) + len(gamma.reshape(-1)) + len(beta.reshape(-1)))
    for t in range(data.shape[1]):
        ...
        for p in range(P):
            ...
            for i in range(M):
                log_likelihood += (data[p,t,i] * (np.log(lambda_total[p]) + np.log(s[p,i])) - lambda_pi[p,i]) * (1/ll_scaling[p])
                
                mod_part = np.sum([(np.int(i==j) - s[p,j]) * return_gradient_of_T_p_i(j, p, t, mu_hat, beta, gamma, xf, lambda_indpt, mu, ext, N_mean, N_scale, X_mean, X_scale) for j in range(M)],axis=0)
    ...
    log_likelihood += regularization * (
        np.sum(np.abs(gamma.reshape(-1))) +
        np.sum(np.abs(beta.reshape(-1)))
    )
    
    # additional regularization on OFF entries
    mask = get_structural_mask_for_gamma(gamma)
    structural_cost_for_gamma = get_structural_cost_for_gamma(gamma, mask)
    structural_gradient_for_gamma = get_structural_gradient_for_gamma(gamma, mask)
    log_likelihood += regularization_structural * structural_cost_for_gamma
    ...
    gradient += regularization_structural * (
        np.hstack([
            np.zeros(shape=len(mu_hat.reshape(-1))),
            structural_gradient_for_gamma,
            np.zeros(shape=len(beta.reshape(-1)))
        ])
    )
...
def return_likelihood_and_gradient_from_flattened(plist, data, X, S, alpha, mu, theta, regularization, regularization_structural, time_averaged):
    P = data.shape[0]
    M = data.shape[-1]
...
def get_likelihood_and_gradient_of_set(samples, X, S, x0, alpha, mu, theta, regularization, regularization_structural, time_averaged, how_long, how_many_samples):
    
    nlls = []
    grads = []
    for i in range(how_many_samples):
        nll, grad = return_likelihood_and_gradient_from_flattened(x0, samples[i,:,:how_long,:], X[:,:how_long], S[:how_long], alpha, mu, theta, regularization, regularization_structural, time_averaged)
        nlls.append(nll)
        grads.append(grad)
...
class MARKETSHAREMODELWITHSAMPLES(object):
    ...
    def __init__(
        ...
        self.regularization_structural = regularization_structural
        self.time_averaged = time_averaged
        self.how_long = how_long
        self.how_many_samples = how_many_samples
        
        _, grad = get_likelihood_and_gradient_of_set(self.samples, self.X, self.S, p0, self.alpha, self.mu, self.theta, self.regularization, self.regularization_structural, self.time_averaged, self.how_long, self.how_many_samples)
        self.grad = grad
        
        logging.basicConfig(filename=f"log/opt2:{logfit_label}.log", level=logging.INFO, format='%(asctime)s | %(message)s', force=True)

    def objective(self, plist):
        start = perf_counter()

        objective, gradient = get_likelihood_and_gradient_of_set(self.samples, self.X, self.S, plist, self.alpha, self.mu, self.theta, self.regularization, self.regularization_structural, self.time_averaged, self.how_long, self.how_many_samples)
        self.grad = gradient
    ...
    def constraints(self, x):
        ...
        #
        return np.array([np.sum([x[p*self.M:(p+1)*self.M]]) for p in range(self.P)])
